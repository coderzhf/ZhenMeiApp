...
// 包括宽高，子控制器视图frame
- (void)calculateSize{
    viewHeight = self.view.height - self.menuHeight;
...
- (void)addViewControllerAtIndex:(int)index{
    ...
    [self.displayVC setObject:viewController forKey:@(index)];
    if (index == 0) {
        self.currentViewController = self.displayVC[@(index)];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    ...
    NSLog(@"%@",[self.currentViewController class]);
    [self postFullyDisplayedNotificationWithCurrentIndex:self.selectIndex];
}
...
- (void)menuView:(WMMenuView *)menu didSelesctedIndex:(NSInteger)index currentIndex:(NSInteger)currentIndex{
    ...
    self.currentViewController = [self.memCache objectForKey:@(_selectIndex)];
    if (self.currentViewController == nil) {
        [self addViewControllerAtIndex:_selectIndex];
        self.currentViewController = [self.displayVC objectForKey:@(_selectIndex)];
    }
    NSLog(@"%@",[self.currentViewController class]);
